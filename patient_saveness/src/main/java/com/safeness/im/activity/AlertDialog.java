...
import com.safeness.im.task.DownloadImageTask;
...
public class AlertDialog extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(path != null){
		    ...
		    if(ImageCache8.getInstance().get(path) != null){
		        imageView.setImageBitmap(ImageCache8.getInstance().get(path));
		    }else{
		        Bitmap bm = ImageUtils.decodeScaleImage(path, 150, 150);
		        imageView.setImageBitmap(bm);
                ImageCache8.getInstance().put(path, bm);
		    }
