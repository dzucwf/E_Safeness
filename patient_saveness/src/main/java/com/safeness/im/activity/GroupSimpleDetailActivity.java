				...
				try {
					...
					runOnUiThread(new Runnable() {
						public void run() {
							pd.dismiss();
							if(group.isMembersOnly())
								Toast.makeText(GroupSimpleDetailActivity.this, "发送请求成功，等待群主同意", Toast.LENGTH_LONG).show();
							else
								Toast.makeText(GroupSimpleDetailActivity.this, "加入群聊成功", Toast.LENGTH_LONG).show();
					...
					runOnUiThread(new Runnable() {
						public void run() {
							pd.dismiss();
							Toast.makeText(GroupSimpleDetailActivity.this, "加入群聊失败：" + e.getMessage(), Toast.LENGTH_LONG).show();
						}
					});
				}
			}
