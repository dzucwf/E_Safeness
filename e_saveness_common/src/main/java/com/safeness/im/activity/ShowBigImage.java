...
import com.safeness.im.task.LoadLocalBigImgTask;
import com.safeness.im.utils.ImageCache8;
import com.safeness.im.widget.photoview.PhotoView;
...
public class ShowBigImage extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (uri != null && new File(uri.getPath()).exists()) {
			...
			bitmap = ImageCache8.getInstance().get(uri.getPath());
			if (bitmap == null) {
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						if (bitmap == null) {
							...
							image.setImageBitmap(bitmap);
							ImageCache8.getInstance().put(localFilePath, bitmap);
							isDownloaded = true;
						}
						if (pd != null) {
							pd.dismiss();
