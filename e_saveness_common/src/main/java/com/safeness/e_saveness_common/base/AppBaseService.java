...
public abstract class AppBaseService extends Service implements ResultCallback {

//    public AppBaseService(String name) {
//		super(name);
        ...
        if (mAsyncRequests != null && mAsyncRequests.size() > 0) {
            ...
            for (AsyncBaseRequest request : mAsyncRequests) {
                ...
                if (conn != null) {
                    try {
//                        conn.disconnect();
                    	Log.i(AppBaseService.class.getName(), "onDestroy disconnect URL: " + conn.getURL());
